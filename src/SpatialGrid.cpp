...
SpatialGrid::SpatialGrid(const float gameWidth, const float gameHeight, const float cellSize)
    : cellSize(cellSize), gridWidth(static_cast<int>(std::ceil(gameWidth / cellSize))), gridHeight(static_cast<int>(std::ceil(gameHeight / cellSize))) {
...
void SpatialGrid::Update(Enemy *enemy) {
  for (const int cellIdx : gridCells) {
